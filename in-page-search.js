    ...
    resultsPanelElement.innerHTML = `
      <div class="in-page-search-results-header">
        ...
        <div class="in-page-search-results-actions">
          ...
          <button
            type="button"
            id="inPageCopySelected"
            class="in-page-search-copy-button"
  ...
  async function handleCopySelectedMatches() {
    ...
    const htmlBody = selected
      .map((mark, index) => {
        ...
        return (
          `<p style="margin:0 0 3px;font-family:Tahoma,Arial,sans-serif;` +
          `font-size:14px;line-height:1.9;color:#1f2937;">` +
          `<strong>${index + 1}.</strong>\u00a0«${snippet}»</p>` +
          `<p style="margin:0 0 14px;font-family:Tahoma,Arial,sans-serif;` +
          `font-size:12px;">🔗 ` +
    ...
    const html =
      ...
      `font-size:14px;font-weight:bold;color:#173b63;">` +
