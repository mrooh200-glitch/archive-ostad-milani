(function () {
  ...
  // ---- Short links (self-hosted, no external dependency) ---------------
  // See the matching comment in index.htm for the full rationale: this
  // redirects long, Persian-heavy #:~:text= links through a tiny local
  // page (s.html) so the copied link is short even in plain-text-only
  // paste targets like Eitaa. Duplicated here (rather than shared via
  // an included script) since book pages aren't all wired to load a
  // common module beyond this file.
  function encodeShortLinkPayload(payload) {
  ...
  function buildShortLinkFromUrl(fullUrl) {
    try {
      const parsed = new URL(fullUrl, location.href);
      const payload = { p: parsed.pathname };

      const hash = parsed.hash || "";
      const textFragmentMatch = hash.match(/^#:~:text=(.+)$/);

      if (textFragmentMatch) {
        payload.t = decodeURIComponent(textFragmentMatch[1]);
      } else if (hash) {
        payload.h = hash;
      }

      // buildMatchUrl() repeats the matched sentence in both ?frag=
      // and #:~:text=. Don't pay for it twice: swap the duplicate for
      // `true` and restore it from payload.t on the redirect page.
      const searchEntries = [...parsed.searchParams.entries()].map(
        ([key, value]) => [key, payload.t && value === payload.t ? true : value]
      );

      if (searchEntries.length) {
        payload.q = searchEntries;
      }

      const encoded = encodeShortLinkPayload(payload);
      return `${location.origin}/s.html#${encoded}`;
    } catch (error) {
      return fullUrl;
    }
  }
  ...
  async function handleCopySelectedMatches() {
    ...
    // The link is routed through the local s.html redirect page (see
    // buildShortLinkFromUrl above), so it stays short even in
    // plain-text-only paste targets (Eitaa, etc.); anywhere rich/HTML
    // paste is supported it additionally collapses to a "لینک منبع"
    // hyperlink label.
    ...
    const textBody = selected
      .map((mark, index) => {
        ...
        const url = buildShortLinkFromUrl(buildMatchUrl(mark));
    ...
    const htmlBody = selected
      .map((mark, index) => {
        ...
        const url = escapeHtml(buildShortLinkFromUrl(buildMatchUrl(mark)));
        return (
