(function () {
  ...
  function renderResultsPanel() {
    ...
    resultsPanelElement.innerHTML = `
      <div class="in-page-search-results-header">
        ...
        <div class="in-page-search-results-actions">
          ...
          <button type="button" id="inPageCopyEitaaAuto" class="in-page-search-copy-button"
            title="کپی برای اسکریپت خودکارسازی ایتا (AutoHotkey/Hammerspoon)" disabled>
            کپی خودکار برای ایتا
          </button>
    ...
    const eitaaAutoButton =
      resultsPanelElement.querySelector("#inPageCopyEitaaAuto");

    if (eitaaAutoButton) {
      eitaaAutoButton.addEventListener("click", handleCopyEitaaAutoMatches);
    }
  ...
  function updateCopySelectedButton() {
    ...
    if (eitaaAutoButton) {
      eitaaAutoButton.disabled = count === 0;
      eitaaAutoButton.textContent = count > 0 ?
        `کپی خودکار برای ایتا (${count})` :
        "کپی خودکار برای ایتا";
    }
  ...
  // ---- Automation payload for the Eitaa AutoHotkey/Hammerspoon script ----
  // Same marker/format as the site-wide search page (index.htm), so one
  // script handles both sources. Fields/records are separated with
  // Private-Use-Area characters (U+E000 / U+E001) that never occur in
  // real article text.
  const EITAA_AUTO_MARKER = "###EITAA-AUTO-LINKS-V1###";
  const EITAA_AUTO_FIELD_SEP = "\uE000";
  const EITAA_AUTO_ITEM_SEP = "\uE001";

  async function handleCopyEitaaAutoMatches() {
    const button = resultsPanelElement.querySelector("#inPageCopyEitaaAuto");
    ...
    const body = selected
      .map(mark => {
        ...
        return [pageTitle, snippet, url].join(EITAA_AUTO_FIELD_SEP);
      })
      .join(EITAA_AUTO_ITEM_SEP);

    const payload = `${EITAA_AUTO_MARKER}\n${body}`;
    const ok = await copyTextToClipboard(payload);

    if (button) {
      button.textContent = ok ?
        "کپی شد! (حالا در ایتا اسکریپت را اجرا کنید)" :
        "خطا در کپی";
      setTimeout(updateCopySelectedButton, 2000);
    }
  }
